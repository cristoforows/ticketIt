...
| D3 | Agent/template compatibility and human-assigned workflow | Templates are not work types. Define supported v1 execution capabilities, repository requirements, and manual completion/rework without deriving completion from engine. Evaluate explicit capability validation and preserving the template-derived completion condition through reassignment. **Status: Proposed, awaiting Owner.** See [decision proposal](decisions/d3-agent-template-compatibility.md) and [#13](https://github.com/cristoforows/ticketIt/issues/13). | [M1 foundational contract](https://github.com/cristoforows/ticketIt/issues/2); apply in M2, M4, and M8 |
